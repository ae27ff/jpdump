...
function addPreviewEXIF(parent,json){
	...
	parent.children[1].children[0].appendChild(elEXIF);
}
...
function displayHeaderInfo(header){
	...
	displayDetail("Actual Size",header.actuallen)+"</ul><ul>"+
	displayDetail("Uses",header.desc.uses)+"</ul><ul>"+
...
function displayJFIFInfo(jfif){
	...
	displayDetail("Y Density",jfif.ydensity)+"</ul><ul>"+
...
function displayEXIFInfo(exif){
	...
	for(prop in exif){
		...
		if(exif[prop] instanceof Array){
			html+=displayDetail(prop,getContentURL(array2string(exif[prop])))+"</ul><ul>"
		}else{
			html+=displayDetailPre(prop,exif[prop])+"</ul><ul>"
		}
		console.log(exif[prop])
		console.log(" ")
	}
...
function displaySOF0Info(sof0){
	var html="<ul>"+
	displayDetail("Data Precision (bits/sample)",sof0.dataprec)+"</ul><ul>"+
	displayDetail("Image Width",sof0.imgw)+
	displayDetail("Image Height",sof0.imgh)+"</ul><ul>"+
...
function displayResults(info){
	...
	for(i=0;i<info.headers.length;i++){
		...
		if(header.hasextendeddata && header.extendedtype==="EXIF"){
			...
			elHeader.children[1].children[0].appendChild(elJFIF);
		}
			...
			if(header.extendeddata.components.length>0){
				for(j=0;j<header.extendeddata.components.length;j++){
					...
					elExtended.children[1].children[0].appendChild(elComponent);
				}
			}
			elHeader.children[1].children[0].appendChild(elExtended);
		}
			...
			elHeader.children[1].children[0].appendChild(elInfo);
		}
